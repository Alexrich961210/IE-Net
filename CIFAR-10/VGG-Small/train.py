...
save_name = 'IE-NEt_vggsmall' #IE-Net_resnet18, IE_Net_vggsmall
...
def main():
    ...
    # -----------------------cosine--------------------------------------
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.epochs-4*args.warm_up, eta_min = 0, last_epoch=-1)



    if args.evaluate:
        validate(val_loader, model, criterion_ce)
        return
    loss_train = []
    acc_train = []
    acc_val = []
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        trainloss,trainacc= train(train_loader, model,criterion_ce,optimizer, epoch)
        ...
        prec1 = validate(val_loader, model, criterion_ce)
